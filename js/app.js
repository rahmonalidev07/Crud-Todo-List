...
function getTime() {
  ...
  const second =
    now.getSeconds() < 10 ? `0` + (now.getSeconds() + 1) : now.getMinutes();
  const months = [
    "January",
    "February",
    "March",
...
function showTodos() {
  ...
  todos.forEach((item, i) => {
    listGroupTodo.innerHTML += `
    <li ondblclick="setCompleted(${i})" class="list-group-item d-flex justify-content-between ${(item.completed =
      true ? "complated" : "")}">
          ${item.text}
...
formCreate.addEventListener("submit", (e) => {
  ...
  if (todoText.length) {
    ...
    showMessage("message-create", "Iltimos biror bir so'z kiriting...");
  }
});
...
formEdit.addEventListener("submit", (e) => {
  ...
  if (todoText.length) {
    ...
    showMessage("message-edit", "Iltimos biror bir so'z kiriting...");
  }
});
// edit todo
